...
public class CartController extends CommomController {
	...
	public String remove(@PathVariable("id") Long id,
		...
		if (product != null) {
			...
			item.setProduct(product);
			item.setId(id);
			// Chỉ cần gọi service remove, không dùng cartItems.remove(session) nữa
	...
	public String checkedOut(Model model,
		...
		if (StringUtils.equals(checkOut, "paypal")) {
			...
			try {
				// Ví dụ: quy đổi VNĐ → USD (tuỳ business logic bạn)
		...
		// sendMail – đổi subject sang GETABEC
		commomDataService.sendSimpleEmail(
				user.getEmail(),
				"GETABEC Shop - Xác nhận đơn hàng",
				"aaaa",
				cartItems,
				totalPrice,
				order
	...
	public String successPay(@RequestParam("paymentId") String paymentId,
		...
		try {
			...
			if ("approved".equals(payment.getState())) {
				...
				commomDataService.sendSimpleEmail(
						user.getEmail(),
						"GETABEC Shop - Xác nhận đơn hàng (PayPal)",
						"aaaa",
						cartItems,
						totalPrice,
						orderFinal
	...
	public String updateToCart(@RequestParam(name = "id") Long id,
